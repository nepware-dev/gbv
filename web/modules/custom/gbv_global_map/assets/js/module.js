(function ($, Drupal, drupalSettings, once) {
  Drupal.behaviors.loadMap = {
    attach(context, settings) {
      const globalMapElement = once("loadMap", "#gbv-global-map", context);
      globalMapElement.forEach(function () {
        ...
        map.on("load", function () {
          postResult.then(function (countries) {
            ...
            mapData.forEach(function (data, index) {
              ...
              filteredCountries.features[index].properties.hrp_country =
                parseInt(data.hrp_country) ? "Yes" : "No";
            ...
            map.on("click", "country-fill", function (e) {
              ...
              const hrp = e.features[0].properties.hrp_country;
              ...
              if (uri !== "null") {
                popupContent += `<a class="global-country-url" href="${
                  uri
                }" target="_blank"> Visit Country Page <i class="fa fa-external-link"></i></a>`;
              }
              popupContent += "</div>";
              if (hrp !== "null") {
                popupContent += `<div class="global-country-header-hrp"><strong>HRP Country:</strong> ${
                  hrp
                }</div>`;
              }
            ...
            map.on("mouseenter", "country-fill", function (e) {
              ...
              const coordinates = centroid.slice();
              const { name } = e.features[0].properties;
              const content = `<p class="country-name">${name}</p>`;
