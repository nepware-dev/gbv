(function ($, Drupal, drupalSettings) {
  Drupal.behaviors.loadMap = {
    attach(context, settings) {
      const gbvRegionalMapElement = once(
        "loadMap",
        "#gbv-regional-map",
        context,
      );
      gbvRegionalMapElement.forEach(function () {
        ...
        const regionalDescription = mapData.region.description
          ? `<p>${mapData.region.description}</p>`
          : "";
        $(document).ready(function () {
          $(
            `<div class="gbv-regional-content-cont"><div class="gbv-regional-content"><h3>${
              mapData.region.name
            }</h3>${regionalDescription}</div></div>`,
        ...
        map.on("load", function () {
          postResult.then(function (countries) {
            ...
            map.on("mouseenter", "country-fill", function (e) {
              ...
              const content = `<h3 class="country-name">${name}</h3>`;
