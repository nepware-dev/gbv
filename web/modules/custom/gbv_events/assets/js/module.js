(function ($, Drupal, drupalSettings, once) {
  Drupal.behaviors.loadCalendar = {
    attach(context, settings) {
      const calenderElement = once("loadCalendar", "#calendar", context);
      calenderElement.forEach(function () {
        if (!document.getElementById("calendar").hasChildNodes()) {
          const calendarEvents = JSON.parse(drupalSettings.calendarEvents);
          $("#calendar").fullCalendar({
            ...
            eventRender(event, element) {
              const { title } = event;
              const { body } = event;
              const { dateContent } = event;
              const uri = event.uri ? event.uri.uri : null;
              const buttonTitle = event.uri ? event.uri.title : null;
              let innerContent =
                `<div class="event-header"><h5>${title}</h5>` +
                `<a class="close-popover" href="javascript:onclick=alert(` +
                `"test"` +
                `)");">&times;</a></div>`;
              ...
              if (uri) {
                innerContent +=
                  `<div class="col-12 nopadding event-footer">` +
                  `<div class="row">` +
        ...
        $(document).on("click", ".event-popover", function (e) {
          e.stopPropagation();
          const btns = document.getElementsByClassName("close-popover");
